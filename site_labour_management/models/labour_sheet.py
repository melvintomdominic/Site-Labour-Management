...
class SiteLabourSheet(models.Model):
    ...
    team_leader_id = fields.Many2one("res.partner", domain=[("parent_id", "=", False), ("company_type", "=", "person")])
    team_labour_ids = fields.Many2many(
        "res.partner",
        string="Team Labour",
        domain="[('parent_id','=',team_leader_id),('company_type','=','person')]",
    )
    ...
    photo = fields.Binary(string="Photo Proof")
    ...
    def _compute_total(self):
        for rec in self:
            rec.total_amount = sum(rec.labour_line_ids.filtered("present").mapped("total"))
    ...
    def _onchange_project(self):
        if self.project_id and hasattr(self.project_id, "account_id") and self.project_id.account_id:
    ...
    def _onchange_team_leader(self):
        ...
        members = self.env["res.partner"].search(
            [("parent_id", "=", self.team_leader_id.id), ("company_type", "=", "person")]
        )
        self.team_labour_ids = [(6, 0, members.ids)]
        if self.attendance_type == "team":
            self.labour_line_ids = [(0, 0, {"labour_id": member.id, "present": True, "days": 1.0}) for member in members]
    ...
    def action_submit(self):
        for rec in self:
            ...
            if not rec.photo:
                raise UserError("Photo proof is required before submitting.")
    ...
    def _create_vendor_bill(self):
        ...
        if not expense_account_id:
            expense_account = self.env["account.account"].search([("internal_group", "=", "expense")], limit=1)
            expense_account_id = expense_account.id if expense_account else 0
        if not expense_account_id:
            raise UserError("No expense account found.")

        invoice_lines = []
        for line in self.labour_line_ids.filtered("present"):
            invoice_lines.append(
                (
                    0,
                    0,
                    {
                        "name": line.labour_id.display_name,
                        "quantity": 1,
        ...
        bill = self.env["account.move"].create(
            {
                ...
                "partner_id": self.supervisor_id.id,
    ...
    def _create_daily_wage_slips(self):
        ...
        for line in self.labour_line_ids.filtered("present"):
            vals = {
                ...
                "days_count": line.days,
                "no_of_labours_worked": 1,
                "basic_wage_day": line.rate,
...
class SiteLabourLine(models.Model):
    ...
    labour_id = fields.Many2one("res.partner", required=True, domain=[("parent_id", "!=", False), ("company_type", "=", "person")])
    category_id = fields.Many2one("site.labour.category", required=True)
    present = fields.Boolean(default=True)
    days = fields.Float(default=1.0)
    ...
    def _onchange_category_id(self):
        self.rate = self.category_id.default_wage

    @api.depends("present", "days", "rate")
    def _compute_total(self):
        for rec in self:
            rec.total = (rec.days * rec.rate) if rec.present else 0.0
